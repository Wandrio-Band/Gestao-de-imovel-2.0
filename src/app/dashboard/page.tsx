...
import { useAssetContext } from '@/context/AssetContext'; // Updated import

const COLORS = ['#10B981', '#1152d4', '#F59E0B', '#EF4444']; // Added Red for others if needed
...
export default function DashboardPage() {
    const { assets } = useAssetContext(); // Use context data

    const totalValue = assets.reduce((acc, curr) => acc + (Number(curr.marketValue) || 0), 0);
    ...
    assets.forEach(asset => {
        asset.partners.forEach(p => {
            const val = (Number(asset.marketValue) || 0) * (p.percentage / 100);
            const pName = Object.keys(partnerValues).find(k => k.includes(p.name)) || p.name;
            partnerValues[pName] = (partnerValues[pName] || 0) + val;
        });
    });
    ...
    const statusCounts = assets.reduce((acc, curr) => {
        const status = curr.status || 'Vago';
        acc[status] = (acc[status] || 0) + 1;
        return acc;
    }, {} as Record<string, number>);

    const occupancyData = [
        { name: 'Locado', value: statusCounts['Locado'] || 0, color: '#10B981' },
        { name: 'Uso Próprio', value: statusCounts['Uso Próprio'] || 0, color: '#1152d4' },
        { name: 'Vago', value: statusCounts['Vago'] || 0, color: '#F59E0B' },
        { name: 'Em Reforma', value: statusCounts['Em Reforma'] || 0, color: '#6B7280' },
    ].filter(d => d.value > 0);

    const occupancyPercentage = assets.length > 0
        ? (((statusCounts['Locado'] || 0) + (statusCounts['Uso Próprio'] || 0)) / assets.length) * 100
        : 0;

    const vacancePercentage = assets.length > 0
        ? ((statusCounts['Vago'] || 0) / assets.length) * 100
        : 0;

    return (
        <div className="p-3 max-w-full mx-auto space-y-3 pb-20">
            <div className="mb-3">
                <h1 className="text-xl font-black text-gray-900 tracking-tight">Visão Geral</h1>
                <p className="text-gray-500 text-xs mt-0.5">Bem-vindo ao hub de gestão patrimonial.</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
                <div className="col-span-1 lg:col-span-2 bg-white rounded-xl p-3 shadow-soft border border-gray-100 relative overflow-hidden group">
                    <div className="absolute right-0 top-0 p-3 opacity-5 group-hover:opacity-10 transition-opacity">
                        <span className="material-symbols-outlined text-9xl">account_balance</span>
                    </div>
                    <div className="relative z-10">
                        <div className="flex items-center gap-2 mb-4">
                            <span className="p-2 bg-gray-50 rounded-[1rem]xt-gray-600"><span className="material-symbols-outlined">payments</span></span>
                        ...
                        <div className="flex items-center gap-2 text-xs">
                            <span className="bg-green-50 text-green-700 px-2 py-1 rounded-full text-xs font-bold flex items-center gap-1">
                                <span className="material-symbols-outlined text-sm">trending_up</span> +0.0%
                            </span>
                            <span className="text-xs text-gray-400 font-medium">vs anterior</span>
                        </div>
                ...
                <div className="col-span-1 bg-white rounded-xl p-3 shadow-soft border border-gray-100 flex flex-col items-center justify-center relative">
                    <h3 className="absolute top-4 left-6 text-xs font-bold text-gray-400 uppercase tracking-widest flex items-center gap-2">
                    ...
                    <div className="h-40 w-full relative mt-4">
                        <ResponsiveContainer width="100%" height="100%">
                            <PieChart>
                                <Pie
                                    data={occupancyData.length > 0 ? occupancyData : [{ name: 'Sem Dados', value: 1, color: '#e5e7eb' }]}
                                    cx="50%"
                                    cy="50%"
                                    innerRadius={40}
                                    outerRadius={60}
                                    ...
                                    stroke="none"
                                >
                                    {(occupancyData.length > 0 ? occupancyData : [{ name: 'Sem Dados', value: 1, color: '#e5e7eb' }]).map((entry, index) => (
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                            <span className="text-2xl font-black text-gray-900">{occupancyPercentage.toFixed(0)}%</span>
                            <span className="text-[10px] font-bold text-gray-400 uppercase">Ocupado</span>
                        </div>
                    </div>
                    <div className="flex justify-between w-full mt-2 px-2">
                        <div className="text-center">
                            <span className="block text-lg font-bold text-gray-900">{assets.length}</span>
                            <span className="block text-[10px] font-bold text-gray-400 uppercase">Ativos</span>
                        </div>
                        <div className="text-center">
                            <span className="block text-lg font-bold text-gray-900">{vacancePercentage.toFixed(1)}%</span>
                            <span className="block text-[10px] font-bold text-gray-400 uppercase">Vacância</span>
                        </div>
                    </div>
                </div>

                <div className="col-span-1 bg-white rounded-xl p-3 shadow-soft border border-gray-100 flex flex-col">
            ...
            {/* IA Insights Section */}
            <div className="bg-[#101622] rounded-xl p-3 lg:p-5 shadow-xl relative overflow-hidden group">
                ...
                <div className="relative z-10 flex flex-col lg:flex-row gap-5 lg:items-center">
                    ...
                    <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="bg-white/5 border border-white/10 rounded-xl p-5 hover:bg-white/10 transition-colors cursor-pointer group/card">
