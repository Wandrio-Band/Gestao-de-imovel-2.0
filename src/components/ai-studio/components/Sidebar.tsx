...
export const Sidebar: React.FC<SidebarProps> = ({ currentView, onNavigate }) => {
    ...
    return (
      <div className="mb-2">
        <button
          ...
          className={`w-full flex items-center gap-2 px-2.5 py-1.5 rounded-md transition-all duration-200 group ${isActive
            ? 'bg-black text-white shadow-lg shadow-gray-200'
            : 'text-gray-500 hover:bg-gray-100'
            }`}
        >
          <span className={`material-symbols-outlined text-[18px] ${isActive ? 'text-white' : 'text-gray-400 group-hover:text-gray-600'}`}>{icon}</span>
  ...
  return (
    <div className="w-48 h-full bg-white border-r border-gray-100 flex flex-col flex-shrink-0">
      <a href="/" className="p-3 flex items-center gap-2 hover:bg-gray-50 transition-colors cursor-pointer">
        <div className="w-7 h-7 bg-black rounded-lg flex items-center justify-center">
          <span className="material-symbols-outlined text-white text-lg">domain</span>
        </div>
        <div className="flex flex-col">
          <span className="font-black text-xs tracking-tighter leading-none">GESTÃO DE</span>
          <span className="font-bold text-[9px] text-gray-400 tracking-widest leading-none">IMÓVEIS</span>
        </div>
      </a>
      ...
      <nav className="flex-1 overflow-y-auto py-2 px-2 space-y-0.5">
        <div className="mb-6">
          ...
          <NavItem
            ...
            subItems={[
              ...
              { view: 'import_irpf', label: 'Importar IRPF' }
            ]}
            relatedViews={['asset_new', 'asset_values', 'import_irpf']}
          />
        ...
        <div className="mb-6">
          ...
          />
          <NavItem view="conciliation" icon="compare_arrows" label="Conciliação Bancária" />
          <NavItem view="adjustments" icon="trending_up" label="Reajustes de Aluguel" />
          <NavItem view="invoices" icon="receipt_long" label="Notas Fiscais" />
          <NavItem view="ai_assistant" icon="smart_toy" label="Assistente IA" />
          <NavItem view="audit" icon="security" label="Logs de Auditoria" />
        </div>

        <div className="mb-6">
          <p className="px-4 text-[10px] font-black text-gray-300 uppercase tracking-widest mb-2">Relatórios</p>
          <NavItem view="partners" icon="pie_chart" label="Sócios & Cotas" />
          <NavItem view="tenants" icon="group" label="Inquilinos" />
          <NavItem view="contracts" icon="history_edu" label="Contratos" />
        </div>
      </nav>

      <div className="p-2 border-t border-gray-100">
        <button className="w-full flex items-center gap-2 px-2 py-1.5 text-gray-500 hover:bg-gray-50 rounded-md transition-all duration-200 group">
          <img src="https://picsum.photos/id/64/100/100" className="w-6 h-6 rounded-full border-2 border-white shadow-sm" alt="User" />
          <div className="flex flex-col items-start">
            <span className="text-[10px] font-bold text-gray-800">Ricardo Silva</span>
            <span className="text-[9px] font-medium text-gray-400">Gestor</span>
          </div>
          <span className="material-symbols-outlined text-gray-300 ml-auto text-sm group-hover:text-gray-600">expand_more</span>
        </button>
      </div>
    </div>
  );
