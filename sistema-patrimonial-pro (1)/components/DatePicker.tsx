...
export const DatePicker: React.FC<DatePickerProps> = ({ label, value, onChange, error, placeholder = 'dd/mm/aaaa' }) => {
  ...
  return (
    <div className="relative" ref={wrapperRef}>
        <label className={`text-[10px] font-bold uppercase block mb-2 ${error ? 'text-red-600' : 'text-gray-500'}`}>{label}</label>
        <div 
            onClick={() => setShowCalendar(!showCalendar)}
            className={`w-full flex items-center justify-between bg-white border border-gray-300 rounded-xl p-3 cursor-pointer hover:border-gray-400 transition-all shadow-sm ${error ? 'bg-red-50 border-red-300 text-red-600' : ''} ${showCalendar ? 'border-black ring-1 ring-black' : ''}`}
        >
            <span className={`text-sm font-bold ${formattedValue ? 'text-gray-900' : 'text-gray-400'}`}>
                {formattedValue || placeholder}
            </span>
            <span className={`material-symbols-outlined text-sm ${showCalendar ? 'text-black' : 'text-gray-400'}`}>calendar_month</span>
        </div>
