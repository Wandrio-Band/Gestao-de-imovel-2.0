...
export const PartnerAudit: React.FC = () => {
  ...
  const [valueBasis, setValueBasis] = useState<'market' | 'acquisition'>('market'); // State for value toggle
      ...
      <div className="flex flex-col lg:flex-row gap-4 mb-6">
          <div className="relative flex-1">
              ...
              <input 
                ...
                className="w-full pl-11 pr-4 py-3 bg-white border border-gray-200 rounded-xl text-sm font-medium focus:ring-2 focus:ring-primary/20 outline-none shadow-sm"
              />
          </div>
          <select className="px-4 py-3 bg-white border border-gray-200 rounded-xl text-sm font-medium text-gray-700 outline-none cursor-pointer hover:bg-gray-50 shadow-sm w-48">
             <option>Todos os Tipos</option>
          </select>
          <select className="px-4 py-3 bg-white border border-gray-200 rounded-xl text-sm font-medium text-gray-700 outline-none cursor-pointer hover:bg-gray-50 shadow-sm w-48">
             <option>Todos os Status</option>
          </select>
          <select className="px-4 py-3 bg-white border border-gray-200 rounded-xl text-sm font-medium text-gray-700 outline-none cursor-pointer hover:bg-gray-50 shadow-sm w-32">
      ...
      <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 flex items-center justify-between mb-8">
          <div className="flex items-center gap-2 text-blue-800">
              ...
              <span className="text-sm font-medium">
                  Total de <span className="font-bold">{filteredAssets.length}</span> imóveis filtrados, <span className="font-black">R$ {formatCurrency(totalFilteredValue)}</span> em cotas ({valueBasis === 'market' ? 'Mercado' : 'Aquisição'})
              </span>
          </div>
                            ...
                            <div className="col-span-4 flex items-center gap-4">
                                ...
                                <div className="flex-1">
                                    <div className="flex items-center gap-2 mb-1">
                                        <span className="bg-gray-100 text-gray-500 text-[9px] font-bold px-1.5 py-0.5 rounded uppercase">ID #{asset.id}</span>
                                    </div>
