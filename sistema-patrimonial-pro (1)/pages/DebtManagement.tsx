...
export const DebtManagement: React.FC<DebtManagementProps> = ({ onNavigate }) => {
  // 1. Calculate Debt Metrics from MOCK_ASSETS
  const assetsWithDebt = MOCK_ASSETS.filter(a => a.financingDetails);
  
  const totalMarketValue = MOCK_ASSETS.reduce((acc, curr) => acc + curr.marketValue, 0);
